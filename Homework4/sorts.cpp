...
{
	// TOD: You implement this.  Using the same criteria as compareSensor,
...
{
	// TOD: Using the insertion sort algorithm (pp. 311-313), sort s
...
int main()
{
	...
	// destruction time.
	{
		...
		// TOD:  Create a vector of Sensor pointers, and set each pointer
		...
		for (vector<Sensor>::iterator i = auxSensors.begin(); i != auxSensors.end(); i++)
			auxSensorsPtr.push_back(&(*i));

		// TOD:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.
		sort(auxSensorsPtr.begin(), auxSensorsPtr.end(), compareSensorPtr);

		// TOD:  Using the now-sorted vector of pointers, replace each Sensor
		//        in sensors with the Sensors from auxSensors in the correct
		//        order.
		sensors.clear();
		for (vector<Sensor*>::iterator i = auxSensorsPtr.begin(); i != auxSensorsPtr.end(); i++)
			sensors.push_back(*(*i));
